...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        ArrayList<Integer> friends = getAllFriendsOfPerson(index, result);
        ArrayList<Integer> friendsOfFriends = new ArrayList<>();
        for (int d = 1; d < deep; d++) {
            for (int i = 0; i < friends.size(); i++) {
                int currentFriend =  friends.get(i);
                if (currentFriend != index) {
                    friendsOfFriends.addAll(getAllFriendsOfPerson(currentFriend, result));
                }
            }
            friends = (ArrayList<Integer>) friendsOfFriends.clone();
    ...
    private ArrayList<Integer> getAllFriendsOfPerson(int index, Set<Integer> set) {
        ...
        for (int i = index; i < humansRelationships.length; i++) {
            if (i != index && humansRelationships[i][index]) {
                result.add(i);
                set.add(i);
            }
        }
